...
# 2026-05-11 第六轮内容优化规划

- 当前 `content/questions/` 下共有 400 篇 Markdown 内容，已按 13 个专题分布：`project 47`、`mysql 45`、`spring 39`、`redis 36`、`distributed 34`、`mq 30`、`jvm 26`、`network 24`、`engineering 22`、`mybatis 15`、`os 14`、`algorithm 12`、`java 56`。
- 现有 400 篇已经完成五轮工作：批量扩写、追问增强、审计、修复、全量校验；因此本轮不宜再做“统一扩写”，而应改成“定向提质”。
- 现阶段最值得优化的不是覆盖率，而是 4 个维度：`模板感`、`专题差异度`、`实战证据密度`、`项目表达自然度`。
- 审计记录已经覆盖 Batch 1-40，因此第六轮可以直接复用 `docs/question-audit/` 的问题清单，不必重新从零盘点。
- 适合把批次粒度从此前的 10 篇提升到 20 篇：因为本轮是精修，不是从零生成，组织成本会更低，但仍保留人工抽检空间。
- 第六轮 Batch 1 的高收益切口已经验证有效：优先改 `一句话结论`、`面试回答`、`常见追问`、`图解提示`，能明显压掉模板味，又不会破坏已有 `详细讲解` 和 visual 数据结构。
- 第六轮 Batch 1 涉及 20 篇 Java 并发/集合高频题，重点从“会背概念”提升到“能讲边界、容量、指标、排障和选型取舍”。
- 当前构建阻塞来自既有前端类型错误 `src/components/personal-center.tsx:588`：`personal` 可能为 `null`。这不是内容回归，但会影响后续任何需要 `npm run build` 作为强验收的批次。
