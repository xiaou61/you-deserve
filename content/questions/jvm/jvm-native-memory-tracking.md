...
就是只讲术语，不讲观测。像「Native Memory Tracking 有什么用」这种题，如果不顺手补一句会看什么日志、火焰图、JFR 或诊断命令，内容就容易飘。
...
别只背定义，可以直接说：如果 Native Memory Tracking 有什么用 相关指标异常，我会先看哪几个信号，再判断是短期抖动还是结构性问题，最后再决定调参数、改代码还是换收集器。
...
## 详细讲解
Native Memory Tracking 有什么用？ 核心结论是：NMT 用来观察 JVM 进程的本地内存分布，排查堆外内存增长。回答时先把它放回JVM语境，说明它解决什么矛盾，再围绕运行时结构、参数开关、日志指标、排障工具和版本差异展开。
...
JVM 题真正好用的答法，一定会把概念和观测工具绑在一起。除了说明 NMT 用来观察 JVM 进程的本地内存分布，排查堆外内存增长，还可以顺手补一句你会看哪些信号，例如 GC 日志、JFR、jstack、NMT、堆外内存曲线或暂停时间分布。用户真正需要的不是再听一遍术语定义，而是知道出问题时第一眼该看哪里。

## 图解提示

适合画一张运行机制图：先放内存区或执行入口 -> 再标关键对象/线程 -> 展开 启动参数开启 NativeMemoryTracking，通常用 summary 或 detail 模式 -> 补一个常见误判点 -> 标观测信号 -> 最后接排查动作。画的时候让“现象”和“底层位置”能对应起来。
